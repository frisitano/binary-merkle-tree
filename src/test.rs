...
fn build_data() -> (Vec<u32>, Vec<<Sha3 as Hasher>::Out>, usize, <Sha3 as Hasher>::Out) {
    let depth = 3usize;
    let values: Vec<u32> = test_values();
    let leaves: Vec<<Sha3 as Hasher>::Out> = values.iter().map(|value| Sha3::hash(&value.to_le_bytes())).collect();
    ...
    let leaf_pairs = unsafe { slice::from_raw_parts(nodes.as_ptr() as *const [<Sha3 as Hasher>::Out; 2], n) };
...
fn build_db_mock() -> (MemoryDB<Sha3, NoopKey<Sha3>, Vec<u8>>, [u8;32], usize) {
    ...
    for (index, value) in values.iter().enumerate() {
        ...
        memory_db.as_hash_db_mut().emplace(key, EMPTY_PREFIX, value.to_le_bytes().to_vec());
    }

    for node_index in 1..nodes.len() {
        let node_index_bytes = node_index.to_le_bytes();
        let key = Sha3::hash(&node_index_bytes);
        memory_db.as_hash_db_mut().emplace(key, EMPTY_PREFIX, nodes[node_index].to_vec());
    }
...
fn test_get_value() {
    ...
    for (i, value) in test_values.iter().enumerate() {
        assert_eq!(u32::from_le_bytes(tree_db.get_value(i).unwrap().try_into().unwrap()), *value)
    }

    let tree_db_mut = TreeDBMut::new(&mut memory_db, &mut root, depth);
    for (i, value) in test_values.iter().enumerate() {
        assert_eq!(u32::from_le_bytes(tree_db_mut.get_value(i).unwrap().try_into().unwrap()), *value)
    }
}
...
fn test_insert_tree_db_mut() {
    ...
    let old_value = tree_db_mut.insert_value(3, new_value.to_le_bytes().to_vec()).unwrap();
...
fn test_commit_tree_db_mut() {
    ...
    let new_value = 67u32;
    let _old_value = tree_db_mut.insert_value(3, new_value.to_le_bytes().to_vec()).unwrap();

    tree_db_mut.commit();

    let expected_root: DBValue = vec![221, 139, 96, 63, 186, 15, 51, 124, 240, 238, 232, 94, 45, 200, 201, 221, 210, 128, 67, 14, 30, 252, 192, 76, 194, 31, 143, 116, 171, 178, 152, 98];
...
fn test_recorder() {
    ...
    let tree_db_builder = TreeDBBuilder::<Sha3>::new(&mut memory_db, &root, depth).with_recorder(&mut recorder);
    let tree_db = tree_db_builder.build();
    ...
    let expected_nodes = BTreeSet::from([17, 15]);
